      ...
      {events.map((event) => {
        ...
        return (
          <div
            ...
            className={`cursor-pointer transition-all duration-500 ease-out group ${isActive ? 'opacity-100 scale-100' : 'lg:opacity-40 lg:scale-95 lg:hover:opacity-60'
              }`}
          >
            <div className="flex flex-col gap-4">
              ...
              <div className="flex items-baseline gap-4">
                <span className={`font-serif text-5xl md:text-6xl font-bold transition-colors ${isActive ? 'text-sepia-900' : 'text-sepia-300'
                  }`}>
                  {event.age}
                </span>
              ...
              <h2 className={`font-serif text-2xl md:text-3xl leading-tight transition-colors ${isActive ? 'text-sepia-900' : 'text-sepia-800/50'
                }`}>
                {event.title}
              </h2>

              <p className={`font-sans text-sepia-800/80 leading-relaxed max-w-prose transition-opacity ${isActive ? 'opacity-100' : 'opacity-0 h-0 overflow-hidden'
                }`}>
                {event.description}
              </p>
            </div>
